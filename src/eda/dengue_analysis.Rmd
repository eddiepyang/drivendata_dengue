...
```{r}
#lme4::lmer(total_cases ~ 1 + (1|city) + station_precip_mm, combined_data2) %>% summary()
# 
# library(MASS)
...
cv_elas1 <- cv.glmnet( dplyr::select(combined_data2, -total_cases, -week_start_date ) %>% data.matrix(),
                dplyr::select(combined_data2, total_cases)[,1] %>% unlist () %>% as.numeric(), 
                family = 'poisson',
                alpha = .5)
