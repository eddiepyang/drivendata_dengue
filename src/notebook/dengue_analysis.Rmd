...
test <- fread('../input/dengue_features_test.csv')
```
...
as.data.frame(as.table(cor_table)) %>%
  subset(abs(Freq) > 0.9 & abs(Freq) < 1)

#cor_table[which(tibble(data.frame(cor_table)) > 0.8)]
...
sj_arima_fit <- auto.arima(sj_series)
